...
class TjsLexicalDfa : public AbstractDfa<LexicalDfaState, char32_t> {
 public:
  ...
  void Transition(char32_t input);
...
class TjsLexicalAnalyzer {
 private:
  ...
  unique_ptr<RunResult> RunOneLine(const u32string &line, size_t line_index);

  enum WhatHappenIfInputThisChar {
    // only used in `RunOneLine` function
    // handling the next action according to the line_buffer and new char

    kDfaContinue,
    // DFA does not reject this char
    // continue to read next char

    kAddNewToken,
    // DFA reject this char, but not reject the previous chars (in line_buffer)
    // add a new token to the token list, and empty the line_buffer

    kLexicalError,
    // DFA reject this char, and line_buffer is empty
    // add a new lexical error to the error list, and empty the line_buffer
  };
  WhatHappenIfInputThisChar GetWhatHappenIfInputThisChar(
