...
void TjsLexicalDfa::Transition(char32_t input) {
...
unique_ptr<RunResult> TjsLexicalAnalyzer::Run() {
  ...
  for (size_t reading_line_index = 0; reading_line_index < kMaxLineIndex;
    ...
    auto line_result = RunOneLine(*reading_line, reading_line_index);
...
unique_ptr<RunResult> TjsLexicalAnalyzer::RunOneLine(const u32string &line, size_t line_index) {
  using std::make_unique;
  using std::string;

  unique_ptr<RunResult> result = make_unique<RunResult>();    // Initialize result.
  dfa_.set_state(kStart); // Initialize DFA state.
  string line_buffer;     // Initialize line buffer.
  ...
  for (size_t reading_column_index = 0; reading_column_index < kMaxColumnIndex;
    ...
    auto next_step = GetWhatHappenIfInputThisChar(line_buffer.empty(),
                                                  GetCharInString(line, reading_column_index));
    switch (next_step) {
      case kDfaContinue:dfa_.Transition(GetCharInString(line, reading_column_index));
        break;
      case kLexicalError:
        result->errors->push_back({line_index,
                                   reading_column_index,
...
#define WHAT_HAPPEN TjsLexicalAnalyzer::WhatHappenIfInputThisChar
WHAT_HAPPEN TjsLexicalAnalyzer::GetWhatHappenIfInputThisChar(
    bool buffer_empty, char32_t new_char) {
  if (dfa_.Transition(dfa_.get_state(), new_char) != kReject) {
    return kDfaContinue;
  } else {
    if (buffer_empty) {
      return kLexicalError;
    } else {
      return kAddNewToken;
    }
  }
